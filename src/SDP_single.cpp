...
// [[Rcpp::export]]
List SDP_single(int tmax, NumericVector res_bs, double cons_bs, int xc, NumericVector rep_gain, NumericVector f_m, List dec_ls, NumericVector rho_vec) {
         ...
         for (int i=0; i<num_dec; i++) {
           ...
           for (int rr=0; rr<num_res; rr++) {
             for (int k=0; k<max_enc; k++) {
               ...
               xp(rr,k) = x + rho_vec(k)*(g_forage(rr) - (c_forage(rr) + c_learn(r,rr))) - (1-rho_vec(k))*(c_forage(rr) + c.learn(r,rr)) 
               ...
               if ((xp(rr,k) < cons_bs) && (xp(rr,k) > xc_state)) {
                 ...
                 W(rr,k) = q(rr,k)*W_nr[r](xp_low(rr,k),t+1) + (1-q)*W_nr[r](xp_high,t+1)
               }
